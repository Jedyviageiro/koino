...
export function VersePickerModal({ visible, books, onClose, onSelect }: { visible: boolean; books: BibleBook[]; onClose: () => void; onSelect: (verse: CommunityVerse) => void }) {
  ...
  async function chooseChapter(next: BibleChapter) {
    ...
    try { setVerses(await getChapterVerses(next.chapterId, pt ? 'NVI' : 'KJV')); }
    finally { setLoading(false); }
  }

  const title = chapter ? `${book ? localizedBibleBook(book.title, language) : ''} ${chapter.chapterNumber}` : book ? localizedBibleBook(book.title, language) : pt ? 'Selecionar versículo' : 'Select a verse';
          ...
          <ScrollView contentContainerStyle={styles.list}>
            {values.map((item) => {
              if ('verseNumber' in item) {
                return <Pressable key={item.verseId} onPress={() => { onSelect({ verseId: item.verseId, reference: `${localizedBibleBook(book!.title, language)} ${chapter!.chapterNumber}:${item.verseNumber}`, text: item.text }); onClose(); }} style={styles.verseRow}><Text style={styles.verseNumber}>{item.verseNumber}</Text><Text style={styles.verseText}>{item.text}</Text></Pressable>;
              }
              if ('chapterNumber' in item) return <Pressable key={item.chapterId} onPress={() => chooseChapter(item)} style={styles.row}><Text style={styles.rowText}>{pt ? 'Capítulo' : 'Chapter'} {item.chapterNumber}</Text><MaterialCommunityIcons name="chevron-right" size={23} color="#737c88" /></Pressable>;
              return <Pressable key={item.bookId} onPress={() => chooseBook(item)} style={styles.row}><Text style={styles.rowText}>{localizedBibleBook(item.title, language)}</Text><MaterialCommunityIcons name="chevron-right" size={23} color="#737c88" /></Pressable>;
            })}
          </ScrollView>
        )}
      </View>
