...
export function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.icon}><MaterialCommunityIcons name="cloud-refresh-outline" size={34} color="#b46d11" /></View>
      <Text style={styles.title}>{language === 'pt' ? 'Vamos tentar novamente' : 'Let’s try that again'}</Text>
